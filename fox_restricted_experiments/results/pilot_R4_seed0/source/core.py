...
class Model:
    ...
    @torch.no_grad()
    def gradients(self, counts=None):
        ...
        weights[self.mask] = dzs * torch.exp(dzl - shift)
        score_grad = -weights
        ...
        finite_logs = dzl[torch.isfinite(dzl)]
        span = finite_logs.max() - finite_logs.min() if finite_logs.numel() else torch.zeros_like(w)

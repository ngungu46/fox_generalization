...
def scalar_error_bounds(delta_min, m, h, rho, w, lag, row_norm_max=1.):
    """Reanalyse a saved checkpoint from its scalar history diagnostics.
    ...
    """
    if m < 0 or w < 0:
        raise ValueError("scalar_error_bounds requires m,w>=0; use the full snapshot for other signs")
    ...
    state = ErrorSnapshot(torch.tensor([delta_min], dtype=torch.float64), *tensors.unbind(),
                          delta_min, row_norm_max, not reasons, ";".join(reasons))
    return error_bounds(state, lag)
...
def evaluate_asymptotics(model, tags, step, S, reference_gap=None,
    ...
    learned = model.cfg.gate_mode == "learned"
    ...
    probes = []
    lags = set(fixed_lags) | {model.cfg.R + offset for offset in range(4)}

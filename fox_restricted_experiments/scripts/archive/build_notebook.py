...
TESTS = ROOT / "tests"
OUT = ROOT / "notebooks" / "archive" / "FoX_Restricted_Generalization_Colab.ipynb"
...
""")
    ...
    for test_name in ("test_core", "test_runner"):
        if (TESTS / (test_name + ".py")).exists():
            names.append(test_name)
    hashes = {}
    for name in names:
        source = ((TESTS if name.startswith("test_") else HERE) / (name + ".py")).read_text(encoding="utf-8")
